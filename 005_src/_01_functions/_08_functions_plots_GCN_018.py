...
def loss_model(
    ...
    """
    calculate the loss for each element and prints input target and prediction
    """
    model.to(device)
    ...
    printif(f"{loss_stat=}",printstat)
    #pdb.set_trace()
    # calculate loss
    if loss_stat is not None:
            loss = calculateLoss(losstat = loss_stat,
                                 reduction = reduction,
                                 out = pred_tensor,
                                 target= target, 
                                 x = x,                                 
            ...
            loss_value = "loss_stat not given, loss not computed"
            
    printif(f"{loss_value=}",printstat)
    return loss_value

# wrapper for all the elements given in dataset
def losses_results_model(
                   model,
                   dataset,
                   device = 'cpu',
                   start_from = 0,
    ...
    loss_values = []
    for i,data in enumerate(tqdm(dataset[start_from:end_at])):
        IDX = i+start_from
        printif("",printstat)
...
def plot_results_model(
    ...
    """
    plot the predictions against target
    shows input, target, prediction, intention
    NOTE: mark the predicting delta only when needed! otherwise the plots will look weird
    """
    if isinstance(dataset,list) == False:
        dataset = [dataset]
        start_from = 0
