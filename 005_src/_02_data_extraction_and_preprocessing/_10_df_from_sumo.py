...
IDList_traffic_light = traci.trafficlight.getIDList()
...
while step <= laststep:
    ...
    # get traffic information
    #traffic_light_color = [traci.trafficlight.getRedYellowGreenState(tlsID) 
                     #for tlsID in IDList_traffic_light]
    #test_list = list(zip(IDList_traffic_light,traffic_light_color))
    ...
    for vehID in IDList:
        ...
        # get next TL 
        # this returns a list with all the coming TL
        next_TL = traci.vehicle.getNextTLS(vehID)
        # get the ID of the first TL, therefore the [0][0]
        if len(next_TL)>=1:
            next_TL_ID = next_TL[0][0]
            time_until_next_switch = traci.trafficlight.getNextSwitch(next_TL_ID) - traci.simulation.getTime()
            TLstate = traci.trafficlight.getRedYellowGreenState(next_TL_ID)
            phase_duration = traci.trafficlight.getPhaseDuration(next_TL_ID)
            phase = traci.trafficlight.getPhase(next_TL_ID)
        else:
            next_TL_ID = None
            time_until_next_switch = None
            TLstate = None
            phase_duration = None
            phase = None        
        ...
        #print (traci.trafficlight.getNextSwitch(next_TL_ID))
