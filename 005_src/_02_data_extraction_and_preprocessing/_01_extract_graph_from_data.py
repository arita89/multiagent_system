...
simulation_max_duration = 200 # Int or None, 
opt = 2 # edges weights options, for now 1 or 2
...
with open(GCN_INPUT_INFO, 'w') as filehandle:
    ...
    filehandle.write(f'\n')
    filehandle.write(f'path input df: {data_path}\n')       
    filehandle.write(f'\n')
    filehandle.write(f'sim_duration (timesteps): {simulation_max_duration}\n')
    ...
    filehandle.write(f'delete temporary files: {delete_tempFiles}\n')
    filehandle.write(f'plotstat: {plotgraphstat}\n')
    filehandle.write(f'\n')
    filehandle.write(f'path GCN input: {file_to_write}\n')
    filehandle.write(f'path GIF: {titleGif}\n')
    filehandle.close()
