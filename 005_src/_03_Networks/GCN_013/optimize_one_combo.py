...
if __name__ == '__main__':
    ...
    ##--------------------------------
    # SELECT COMBINATION TO IMPROVE
    ##--------------------------------  

    # overview of the best combinations
    l = len(reloaded_best_combos.keys())
    print(f"FOUND {l} COMBINATIONS TO BE OPTIMIZED")
    for k,v in reloaded_best_combos.items():
        print (f"{k}:{v[0]}")
    
    print (f"which combination you want to improve? eg 20210722-22h15m03s")
    # input combo
    selected_date_time = input()#add here the key data and time of the combination you want to rerun!
    assert selected_date_time in reloaded_best_combos.keys()
    v = reloaded_best_combos[selected_date_time]
    combo = v[0]
    print (f"SELECTED COMBINATION: {combo}")
    print ("")
    #combo = v[0]
    ...
    origing_input_file_name = v[1]["input_file_name"]
    random_seed = v[1]["random_seed"] 
    train_size = v[1]["train_size"]  
    orig_Nepochs = v[1]["Nepochs"]
    orig_batch_size = v[1]["batch_size"] 
    ...
    print (f"model_{GCN_MODEL} {k} was trained with {orig_Nepochs} epochs")
    print (">>>for how many epochs do you want to train now?")
    Nepochs = int(input())
    save_every = int(Nepochs/5) 
    ...
    print (f"model_{GCN_MODEL} {k} was trained with input: {origing_input_file_name}")
    ...
    print (f"model_{GCN_MODEL} {k} was trained with batch_size: {orig_batch_size}")
    ...
    plotstat = True
    run_unattended = True
    run_unattended = set_automatic_run(run_unattended,l)
    final_print = False
