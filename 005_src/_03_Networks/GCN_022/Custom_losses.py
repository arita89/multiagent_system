...
#TO DO: TEST this
def in_crossing(x1, y1,h_wide,v_wide,xc,yc) :
    """
    #check if point is inside the actual crossing
    # central vertical lane centered in xc, wide v_wide
    # central horizonal lane centered in yc, wide h_wide
    """
        
    if (xc-v_wide/2<= x1 <= xc+v_wide/2) or (yc-h_wide/2<= y1 <= yc+ h_wide/2):
        return True
    else:
        return False 

def get_loss_weight_2( out_vect,h_wide,v_wide,xc,yc) :
    """
    #x_vect = initial coordinates
    #out_vect = predicted coordinates
    """
    if in_crossing( out_vect[0], out_vect[1],h_wide,v_wide,xc,yc) :
        return 1
    else:
        return 10**3


class Custom_Loss_7(torch.nn.Module):
    """
    #h_wide= how wide is the central horizontal line of the crossing
    #v_wide = how wide is the central vertical line of the crossing
    #zero_centered_cross = if the outputs are data_y_zc then it is zero centered
    """

    def __init__(self):
        super(Custom_Loss_7,self).__init__()
    ...
    def forward(self,
        ...
        tot_loss = [mse(tv,ov)*get_loss_weight_2(ov,h_wide,v_wide,xc,yc) 
                        for (ov,tv,xv) in zip(out,target,x)]
...
class Custom_Loss_6(torch.nn.Module):
    ...
    handmade loss, using MSE as basis, given the current position and the 
        - max speed of veh (which is 25m/s in the sim)
    ...
    where r = v_max* delta_t = 25m/s*2s = = 50m 
    """
...
class Custom_Loss_4(torch.nn.Module):
    ...
    handmade loss, using L1 formula, given the current position and the 
        - max speed of veh (which is 20km/h in the sim) <-- WRONG!
...
def calculateLoss(losstat,
    ...
    elif losstat == "CrossEntropy":
        criterion = torch.nn.CrossEntropyLoss()
        printif ("\nFLATTEN",printstat)
        printif(target.long().flatten(0).size(),printstat)
        printif(target.long().flatten(0),printstat)
        printif(out.flatten(0).size(),printstat)
        printif(out.flatten(0),printstat)
        printif ("\nVIEW",printstat)
        printif(target.long().view(-1).size(),printstat)
        printif(target.long().view(-1),printstat)
        printif(out.view(-1).size(),printstat)
        printif(out.view(-1),printstat)
        pdb.set_trace()
        loss = criterion(out.flatten(0),target.long().flatten(0)) 
