...
from config import *
from _03_Networks.GCN_015.GCN_model_015 import *
from _03_Networks.GCN_015.GCN_trainer_015 import *
from _03_Networks.GCN_015.GCN_parameters_015 import *
...
def main(
    ...
    #For doing MinMax Normalization
    #df_input_min_max_normalized=min_max_normalize_columns(df_input)
    #df_input=df_input_min_max_normalized
    
    #For doing Standardization
    #df_input_standard_normalized=standard_normalize_columns(df_input)
    #df_input_standard_normalized=standard_normalize_graph(df_input)
    #df_input=df_input_standard_normalized
    ...
    if len(datasets_list) == 5:                               
        ...
        dict_text_output['shuttle_train_frame'] = shuttle_train
        dict_text_output['shuttle_val_frame'] = shuttle_val
        #dict_text_output['shuttle_train_frame'] = shuttle_train_frames[0]
        #dict_text_output['shuttle_val_frame'] = shuttle_val_frames[0]
        ...
        dict_text_output['dateset_train_path'] = datasets_list_paths[0]
        dict_text_output['dateset_val_path'] = datasets_list_paths[1]       
        dict_text_output['dateset_test_path'] = datasets_list_paths[2]
    ...
    for select_column in input_columns:
        if select_column == "data_x_rad":
            size_input += 3
        elif select_column == "data_x_rad_norm":
            size_input += 3
            
        elif select_column == "data_x":
            size_input += 5 #only with dataset that uses tl and duration until traffic light switches
            #size_input += 3 # only with dataset that uses speed, yaw, intention
            
        elif select_column == "data_x_rad_norm_graph":
            size_input += 3    
            
        elif select_column == "data_pos_zc_norm_graph":
            size_input += 2          
        elif select_column == "data_pos_zc":
            size_input += 2
        elif select_column == "data_pos_zc_norm":
            size_input += 2
        elif select_column == 'Still_vehicle':
            size_input +=1
        elif select_column == 'data_x_acceleration':
            size_input +=1
        else: 
    ...
    for select_column in output_columns:
        if select_column == "data_y_zc":
            size_output += 2
        elif select_column == "data_y_delta":
            size_output += 2
        elif select_column == "data_y_zc_norm":
            size_output += 2
    ...
    #### RELU             
        ...
        if num_hidden_layers == 1:
            ...
            print('TEST IF ONE HIDDEN LAYER IS USED')
            hc_1 = hidden_layers_sizes[0]
            hc_2 = None
            hc_3 = None
    ...
    if final_print:
        ...
        for data in dataset_val[:1]:
            ...
            print (np.around(data.x.detach().np(),2))
            print ("\nTARGET DATA_Y")
            print (data.y.detach().np())
            print ("\nPREDICTIONS")
            model.to('cpu')
            if edges_attr:
                print (model(data.x, data.edge_index, data.edge_attr)).detach().detach().np()
            else:
                print (model(data.x, data.edge_index).detach().detach().np())
